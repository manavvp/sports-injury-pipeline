#!/usr/bin/env bash
# Deploy and run Job 1 (Dimensions).
#
# Usage:
#   ./deploy_job1.sh upload   # upload script to S3 only
#   ./deploy_job1.sh sync     # upload + create-or-update Glue job definition
#   ./deploy_job1.sh run      # start a job run (assumes job already exists)
#   ./deploy_job1.sh all      # upload + sync + run, then tail status
set -euo pipefail

BUCKET="sports-injury-pipeline-manav"
JOB_NAME="job1_dimensions"
ROLE_NAME="GlueServiceRole"
SCRIPT_LOCAL="$(dirname "$0")/job1_dimensions.py"
SCRIPT_S3="s3://${BUCKET}/scripts/job1_dimensions.py"
...
sync_job() {
  # create-or-update pattern: if get-job succeeds, update-job; else create-job.
