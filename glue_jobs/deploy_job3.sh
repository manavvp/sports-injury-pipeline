#!/usr/bin/env bash
# Deploy and run Job 3 (Football Facts).
#
# Depends on Job 1 (dimensions) having completed.
#
# Usage:
#   ./deploy_job3.sh upload   # upload script to S3 only
#   ./deploy_job3.sh sync     # upload + create-or-update Glue job definition
#   ./deploy_job3.sh run      # start a job run (assumes job already exists)
#   ./deploy_job3.sh all      # upload + sync + run, then tail status
set -euo pipefail

BUCKET="sports-injury-pipeline-manav"
JOB_NAME="job3_football_facts"
ROLE_NAME="GlueServiceRole"
SCRIPT_LOCAL="$(dirname "$0")/job3_football_facts.py"
SCRIPT_S3="s3://${BUCKET}/scripts/job3_football_facts.py"
