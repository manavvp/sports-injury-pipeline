...
from transforms.nfl_facts import (
    build_fact_injury_event_nfl,
    build_fact_nfl_performance_seasonal,
    build_fact_nfl_performance_weekly,
)
...
def main() -> None:
    ...
    try:
        ...
        raw_injuries_df = read_catalog(glue_context, "nfl_injuries")
        fact_injury_nfl = build_fact_injury_event_nfl(
        ...
        raw_stats_df = read_catalog(glue_context, "nfl_player_stats")
        fact_perf_weekly = build_fact_nfl_performance_weekly(
            raw_stats_df, dim_player_df
        ).cache()
